    ...
    T: Serialize + DeserializeOwned + Message + Send + Sync + 'static,
{
    ...
    pub(crate) fn event_publisher(&self) -> Arc<Publisher<NodeParameterEvent>> {
...
pub(crate) fn handle_get_snapshot_for_state<T>(state: &Arc<ParameterState<T>>, query: Query)
where
    T: Serialize + DeserializeOwned + Message + Send + Sync + 'static,
{
...
pub(crate) fn handle_get_value_for_state<T>(state: &Arc<ParameterState<T>>, query: Query)
where
    T: Serialize + DeserializeOwned + Message + Send + Sync + 'static,
{
...
pub(crate) fn handle_get_type_info_for_state<T>(state: &Arc<ParameterState<T>>, query: Query)
where
    T: Serialize + DeserializeOwned + Message + Send + Sync + 'static,
{
    reply(
...
pub(crate) async fn handle_set_for_driver<T>(driver: &ParameterDriver<T>, query: Query)
where
    T: Serialize + DeserializeOwned + Message + Send + Sync + 'static,
{
...
pub(crate) async fn handle_set_atomic_for_driver<T>(driver: &ParameterDriver<T>, query: Query)
where
    T: Serialize + DeserializeOwned + Message + Send + Sync + 'static,
{
...
pub(crate) async fn handle_reset_for_driver<T>(driver: &ParameterDriver<T>, query: Query)
where
    T: Serialize + DeserializeOwned + Message + Send + Sync + 'static,
{
...
pub(crate) async fn handle_reload_for_driver<T>(driver: &ParameterDriver<T>, query: Query)
where
    T: Serialize + DeserializeOwned + Message + Send + Sync + 'static,
{
...
mod tests {
    ...
    use crate::{context::ContextBuilder, entity::EntityKind, node::Node};
    ...
    async fn callback_replies_busy_when_mailbox_is_full() -> TestResult {
        ...
        assert_busy_write_response(response);
        Ok(())
    }
    ...
    async fn wait_for_service(node: &Node, service: &str) -> TestResult {
        ...
        while start.elapsed() < timeout {
            if node.graph().count(EntityKind::Service, service) >= 1 {
                return Ok(());
            }
    ...
    fn assert_busy_write_response(response: SetNodeParameterResponse) {
        assert!(!response.success);
        assert_eq!(response.message, PARAMETER_ACTOR_BUSY);
        assert_eq!(response.committed_revision, 0);
        assert!(response.changed_paths.is_empty());
