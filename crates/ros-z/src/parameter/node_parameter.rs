...
use super::{
    ...
    remote::{
        RemoteParameterServices, services,
...
pub(crate) struct NodeParametersInner<T>
where
    T: Serialize + DeserializeOwned + Message + Send + Sync + 'static,
{
    state: Arc<ParameterState<T>>,
    commands: parking_lot::Mutex<Option<flume::Sender<ParameterCommand<T>>>>,
    remote: parking_lot::Mutex<Option<RemoteParameterServices<T>>>,
    driver_task: parking_lot::Mutex<Option<tokio::task::JoinHandle<()>>>,
    binding_state: Arc<parking_lot::Mutex<bool>>,
}

pub(crate) struct ParameterState<T> {
    ...
    pub(crate) current: ArcSwap<NodeParametersSnapshot<T>>,
...
pub(crate) enum ParameterCommand<T>
where
    T: Serialize + DeserializeOwned + Message + Send + Sync + 'static,
{
    SetJson {
...
pub(crate) struct ParameterDriver<T>
where
    T: Serialize + DeserializeOwned + Message + Send + Sync + 'static,
{
    state: Arc<ParameterState<T>>,
    commands: flume::Receiver<ParameterCommand<T>>,
    event_publisher: Option<Arc<Publisher<NodeParameterEvent>>>,
}
...
impl Node {
        ...
        T: Serialize + DeserializeOwned + Message + Send + Sync + 'static,
    {
        ...
        match build_parameter_actor(self, parameter_key).await {
            Ok((parameters, driver)) => {
                let task = tokio::spawn(driver.run());
                *parameters.inner.driver_task.lock() = Some(task);
                Ok(parameters)
            }
            Err(err) => {
...
) -> Result<(NodeParameters<T>, ParameterDriver<T>)>
where
    T: Serialize + DeserializeOwned + Message + Send + Sync + 'static,
{
    ...
    let inner = Arc::new(NodeParametersInner {
        state: state.clone(),
        commands: parking_lot::Mutex::new(Some(command_tx.clone())),
        remote: parking_lot::Mutex::new(None),
        driver_task: parking_lot::Mutex::new(None),
        binding_state: self_binding_state(node),
    });
    ...
    T: Serialize + DeserializeOwned + Message + Send + Sync + 'static,
{
    ...
    fn command_sender(&self) -> Result<flume::Sender<ParameterCommand<T>>> {
        ...
        self.inner
            ...
            .as_ref()
            .cloned()
            .ok_or_else(actor_unavailable_error)
    }
    ...
    T: Serialize + DeserializeOwned + Message + Send + Sync + 'static,
{
    ...
    pub(crate) async fn reload_with_source(
        &self,
        source: NodeParameterChangeSource,
    ) -> Result<CommitOutcome> {
        let (outcome, event) = {
    ...
    async fn publish_event(&self, event: &NodeParameterEvent) -> Result<()> {
        if let Some(event_publisher) = &self.event_publisher {
            event_publisher.publish(event).await.map_err(|source| {
                ParameterError::operation("publishing parameter event", source)
            })?;
        }
        Ok(())
    }
...
mod actor_tests {
    ...
    async fn local_mutation_reports_error_after_driver_task_aborts() {
        ...
        {
            let driver_task = parameters.inner.driver_task.lock();
            driver_task
                .as_ref()
                .expect("driver task should be present")
                .abort();
        }
