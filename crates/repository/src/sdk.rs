...
use log::info;
use tokio::{
    fs::{remove_dir_all, remove_file, rename, set_permissions, File},
    process::Command,
};
...
async fn remove_incomplete_sdk(sdk_home: impl AsRef<Path>, version: &str) -> Result<()> {
    let installation_directory = sdk_home.as_ref().join(version);
    let incomplete_marker = sdk_home.as_ref().join(format!("{version}.incomplete"));
