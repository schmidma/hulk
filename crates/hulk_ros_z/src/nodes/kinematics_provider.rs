...
pub async fn run(ctx: Arc<ZContext>) -> Result<()> {
    ...
    let robot_kinematics_pub = node
        .create_pub::<RobotKinematics>("state/robot_kinematics")
        .build()
        .into_eyre()?;

    let mut last_robot_kinematics = RobotKinematics::default();

    loop {
        let low_state = low_state_sub.async_recv().await.into_eyre()?;

        if let Ok(serial_motor_states) = low_state.serial_motor_states() {
            last_robot_kinematics = compute_robot_kinematics(&serial_motor_states.positions());
        }

        robot_kinematics_pub
            .async_publish(&last_robot_kinematics)
            .await
            .into_eyre()?;
    }
}

fn compute_robot_kinematics(measured_positions: &Joints) -> RobotKinematics {
    // head
    ...
    // left arm
    let left_inner_shoulder_to_robot = left_inner_shoulder_to_robot(&measured_positions.left_arm);
    ...
    let right_forearm_to_robot =
        right_upper_arm_to_robot * right_forearm_to_right_upper_arm(&measured_positions.right_arm);
